...
- [x] R1 Triage router + effort controller — 2026-08-14 *(`zilla/router.py`: class (command/share/trivial/normal) + effort (fast/standard/deep) decided by fixed rules before the lock, never by the model. Owner emphasis and a leading `!deep` beat the classifier absolutely and the marker is stripped before the model sees it. `effort_map` maps effort → backend:model and is validated at settings-write time — an entry naming agy is refused with a reason, because agy's model is one global string shared with every agy terminal. A fast turn runs lean (core MEMORY.md, no wiki index) in a throwaway conversation and never advances the session conv id; if it comes back empty or error-shaped it is silently rerun as a normal turn. `test_router.py` (105).)*
- [x] R2 Fallback chain — 2026-08-14 *(`zilla/chain.py` + the walk inside `core.handle_message`: on an error channel only, the turn is retried down `backend_chain` (default agy → opencode → claude, filtered to installed), each entry gated on a fresh `health.login_ok` probe, each attempt a throwaway conv carrying one primer line, delivered as one answer with `↷ answered via <backend>`. `usage.fallbacks` bumped per attempt; exhausted ⇒ one plain sentence. The shape gate moved into `review()`: a limit signal only counts in a short error-shaped response, so a long correct answer about 429s is no longer replaced by a rate-limit notice. `test_chain.py` (61).)*
- [x] R3 opencode adapter — 2026-07-19
- [x] S  Skills from chat — 2026-08-14 *(`zilla/skills.py` (pure: `SKILL_PROPOSAL:` parse/strip, slugify + command-name collisions, folder read, sha256 approval hash over SKILL.md + every other byte, the state machine, the copy) + `skill_approvals.enabled` + `core.Skills`. Ask-first: the marker is an OFFER, owner-turn-only and never in incognito; ✅ arms a one-shot write instruction that rides the owner's NEXT prompt; after that turn Zilla reads what was written — Markdown-only goes live on the tap that authorized it, anything carrying a script stays unapproved until a second explicit tap in `/skills`. The INDEX is the enforcement: only skills with a live approval row whose hash still matches disk are ever named to the model or turned into a command, so a SKILL.md that appeared out-of-band is invisible. Editing an approved skill's bytes auto-switches it off with one owner line. Every approved skill is also an owner-scope slash command running the skill's wording through the same `_run_text_turn` pipeline as typed text. Legacy backend-native skills coexist, ungated, but became owner-turn-only. `test_skills.py` (152).)*
- [x] C1 Brain export/import — 2026-08-17 *(`zilla/brain.py` + `zilla export [--encrypt] [path]` / `zilla import <archive|dir>` + install.py's onboarding "Restore from a backup?" (`--restore <path>` non-interactively). One `.tar.gz`: `Memory/` with its own `.git`, `System/state-snapshot.json` (settings · schedules · users · curiosity), `System/manifest.json`, a keys-only `.env.template`, and `Media/` files under `export_media_max_mb` (10) each with the oversize ones named back to the owner. The database is NOT in the archive: the graph and the FTS index are rebuilt from the Markdown on import (`graph.rebuild` + `Store.fts_clear` then `memory.reindex`), which is the P1 proof — the round-trip test asserts identical nodes/aliases/edges and identical `neighbors()`/`find_path()`/search answers after a full wipe. Curiosity travels keyed by wiki page, not node id, and only restores `asked_at` for a gap the rebuild actually detected. Three things deliberately stay behind: secrets (credential-shaped settings dropped, `.env` reduced to key names), skill approvals (an import must never arrive with code pre-authorized — the owner re-taps), and sessions (conv ids belong to the backend, §17/F1.3). `--encrypt` is AES-256-CBC + pbkdf2 via openssl with the passphrase on stdin, never argv; the plaintext archive never outlives the encrypted one. Import moves the existing `Memory/` into `Runtime/Replaced/` rather than deleting it, never overwrites local media, and refuses a tarball with a `..` member. `test_brain.py` (116).)*
