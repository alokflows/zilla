...
**NEXT UNIT OF WORK: Phase H3 — systemd Linux service deployment**
(PLAN.md §6/H3: `install.py --service` writes/enables a systemd user unit;
`zilla doctor` gets a service-status check; Mac dev keeps `./start.sh`
unchanged). Full test gate before and after.
...
34 memory_m3 + 31 memory_m4 + 61 heartbeat + 57 health = **835 green** —
...
test_heartbeat.py / test_health.py`
(test_schedules_seam.py is a frozen acceptance spec — never edit it) +
`import bot; import zilla.core; import zilla.health; import zilla.heartbeat`.
