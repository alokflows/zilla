...
**NEXT UNIT OF WORK: Phase H1 — Beat loop** (PLAN.md §6/H1: `system`-schedule
try-acquire-with-skip runner, `heartbeat.py` 30-min beat, HEARTBEAT.md
template + deterministic skip-if-untouched check). Full test gate before
and after.
...
34 memory_m3 + 31 memory_m4 = **717 green** — `.venv/bin/python
...
test_memory_m3.py / test_memory_m4.py` (test_schedules_seam.py is a frozen
