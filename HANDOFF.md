...
see checklist + session log below.
**NEXT UNIT OF WORK: K1-K4 (graph memory), THEN K5 (team
...
session.) **F5 session note: a fresh per-file recount landed at 1014
green, not the "1000+4" arithmetic you'd expect from 4 new test
...
**Tests (fresh per-file recount, 2026-07-18 F5 session — this line had gone
stale across F2/F3/F4 and is now the ground truth again):** 291 fixes +
16 interactive + 116 core + 57 schedules_seam + 71 review + 17 tui +
150 cli + 49 harness + 34 memory_m3 + 31 memory_m4 + 67 heartbeat +
57 health + 23 service + 10 quickfix + 25 zilla_home = **1014 green** —
...
test_zilla_home.py` (test_schedules_seam.py is a frozen acceptance spec —
never edit it) + `import bot; import zilla.core; import zilla.cli;
import zilla.tui.app; import schedule_query`.
