...
**NEXT UNIT OF WORK: Phase F (F1→F4)** — foundation cleanup (PLAN.md
§17): F1 ZILLA_HOME storage layout replaces AGI-Brain, F2 dynamic backend
registry (+ slash-command registry), F3 media importance+retention, F4
system jobs invisible+silent. THEN K1-K4, U1-U4, H4, B1-B2, THEN the
...
10 quickfix = **868 green** — `.venv/bin/python test_fixes.py /
...
/ test_service.py / test_quickfix.py`
