# ZILLA — HANDOFF ARCHIVE (history only)

> **This is the historical record, not the brief.** The live brief is
> [`../../HANDOFF.md`](../../HANDOFF.md) at the repo root — short by design, and the
> only handoff file a session needs to read at start.
>
> Everything below is kept for one reason: when a question comes up about
> *why* something was built the way it was, the answer is probably in here
> (full per-phase session log, every owner decision, every trap found).
> Search it; don't read it top to bottom.
