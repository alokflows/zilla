...
> ⚠️ **Part B pending**: the retry-ladder and payload_type/session/backend-pin schema described below are SPEC, not yet implemented — the executor's Part B was reverted (see HANDOFF night wrap-up row). `test_schedules_seam.py` is the red-by-design acceptance suite for it; current code uses the flat retry (`_SCHED_RETRY_DELAY=120`, `_SCHED_MAX_RETRIES=3` in `zilla/core.py`) and the old `mark_failure(sid, retry_delay, max_retries)`.
Runtime lives in `zilla/core.py` (moved from `bot.py` in the P1 scheduler seam; CORE_API migration step 3) — custom asyncio loop, NO APScheduler / PTB JobQueue. `core.start()`→`_scheduler_loop`: `reconcile_startup(catchup=setting schedule_catchup default True)` then ~20s tick; `_execute_schedule` runs under the same per-uid lock the live-chat turn pipeline uses, classifies ok/failure (no delivery, no mutation); `_run_and_record` (tick path) records the outcome and broadcasts a `ScheduledResult` event via `core.subscribe()` — bot.py's `_deliver_scheduled_result` renders it as `⏰ Scheduled — <title>` + result, same as before. `core.stop()` (wired to PTB `post_shutdown`) cancels the loop cleanly. Catch-up = run missed once on boot.

Failure handling: `mark_failure` retries on `RETRY_LADDER` (30s/60s/5m/15m/60m) before giving up; give-up resets and advances to the next normal occurrence (a daily job that fails today still runs tomorrow — never permanently disabled) and broadcasts a `ScheduledResult.warning`. Manual ▶️ trigger is `core.run_schedule_now(sid)` — runs + broadcasts, never advances the schedule.

Schema (see `zilla/schedules.py` module-map entry): `payload_type` (`message` default = full CLI turn; `system_event` = deliver stored text verbatim, zero CLI call; `command` = subprocess, zero model call, owner-only at creation), `session` (isolated/main/named:\<x\>, default isolated), pinned `backend`/`model` (default unpinned) — a mismatch at fire time runs on the CURRENT backend (no per-call override exists in `cli_engine`) and sends the owner a one-time `Alert`, never blocks the run. Recursion guard: `core.is_scheduled_run(uid)` is true while a schedule's own turn executes; bot.py's NL schedule-detection (`parse_schedule`) checks it so a schedule-triggered turn can't create more schedules.
...
## Verification
`python test_fixes.py` (192), `python test_interactive.py` (16), `python test_core.py` (29, turn pipeline) — all deterministic, no-network; isolate state via `AGY_SETTINGS_FILE`/`BACKEND` env + a redirected `config.SETTINGS_FILE` set BEFORE importing `zilla.config`. `python install.py --doctor` (env/login/token/deps). Live smoke: agy+claude round-trip, resume continuity, cancel-no-bleed, schedule fire+catchup, model read-back, menu staleness, no console flash.
