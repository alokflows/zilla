...
- `zilla/core.py` — `ZillaCore` (docs/dev/CORE_API.md): interface-agnostic turn pipeline (`handle_message`, per-user `asyncio.Lock`/`get_user_lock`, `_conv_for_run`, cancel events) **and** the scheduler runtime (`start`/`stop`→`_scheduler_loop`, `_execute_schedule`, `_run_and_record`, `run_schedule_now`; `reconcile_startup`/`due`/`touch_run`/`mark_success`/`mark_failure` calls into `zilla/schedules.py`). Background events (`ScheduledResult`, `Alert`) fan out via `subscribe(sink: asyncio.Queue)` — frontends own rendering, core owns nothing Telegram-specific. `schedule_pre_run` is an injectable frontend fast-path hook (bot.py wires the screenshot-via-bridge shortcut here). `is_scheduled_run(uid)` is the recursion guard: true while a schedule's own turn is executing for that uid.
- `bot.py` — handlers, inline-keyboard state machine, delivery (`send_response`/`keep_typing`, `_deliver_scheduled_result`/`_deliver_alert` as `core.subscribe` renderers), lifecycle (`main`/`post_init`/`post_shutdown`/`_register_commands`), WebBridge async shims (`bridge_status`/`bridge_command` via `asyncio.to_thread`). No longer owns any turn-pipeline or scheduler-runtime state — both live in `zilla/core.py`.
...
- `config.py` — `.env` loader, path resolution (`_find_exe`: PATH→platform default), backend selection (`get_backend/set_backend`), model layer (`get_model/set_model/model_catalog`, backend-dispatched), settings KV (`get_setting/set_setting`, mtime-cached), `BOT_VERSION`.
...
## Verification
`python test_fixes.py` (196), `python test_interactive.py` (16), `python test_core.py` (29, turn pipeline), `python test_schedules_seam.py` (57, scheduler seam Part A+B) — all deterministic, no-network; isolate state via `AGY_SETTINGS_FILE`/`BACKEND` env + a redirected `config.SETTINGS_FILE` set BEFORE importing `zilla.config`. `python install.py --doctor` (env/login/token/deps). Live smoke: agy+claude round-trip, resume continuity, cancel-no-bleed, schedule fire+catchup, model read-back, menu staleness, no console flash.
